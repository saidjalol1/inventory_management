...
const emit = defineEmits(['update:modelValue','update:dateFilter']);
const search = ref(props.modelValue || '');
const date  = ref(props.dateFilter  || '');
...
const onDateChange = () => {
  emit('update:dateFilter', date.value);
};
...
<template>
        ...
        <div v-if="isFilterVisible" class="filter flex justify-around " >
            <input  v-model="date"  @input="onDateChange" type="date" class="search-input"/>
        </div>
</template>
<style scoped>
