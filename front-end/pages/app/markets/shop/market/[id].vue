...
const fetchSales = async () =>{
    try {
      const response = await fetch(`${base}/sales/market/${ route.params.id }`, {
        method: 'GET',
        headers: {
...
const totalDebts = computed(() => {
  return filteredsales.value.reduce((total, sale) => total + (sale.debt || 0), 0);
});
const f = ref(true)
</script>
<template>
    ...
    <div class="mt-6 bg-white rounded-lg p-4 shadow-md">
        <p class="text-lg font-semibold">Jami Qarz: {{ formatPrice(totalDebts) }} so'm</p>
    </div>
